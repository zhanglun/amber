...
创建项目根目录的 `.env` 文件（已在 `.gitignore` 中）：
...
```bash
# 若已有 .env 文件，用 dotenv-cli 加载（推荐）
dotenv pnpm amber web

# 或直接内联
DATABASE_URL=postgres://amber:yourpassword@localhost:5432/amber pnpm amber web
```
...
```bash
# 先预览（不写入数据库）
DATABASE_URL=postgres://... pnpm amber migrate --dryRun

# 实际迁移
DATABASE_URL=postgres://... pnpm amber migrate
```
