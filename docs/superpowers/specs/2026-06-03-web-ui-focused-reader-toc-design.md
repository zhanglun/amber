...
- 空标题忽略。
- heading 文本先做轻量 inline Markdown 清理，再用于 TOC 文本和 slug。
...
- 基于清理后的 heading 纯文本 slug。
- 英文 / 数字转小写，空格和标点转 `-`。
- 中文保留，保证中文标题可读。
- 重复标题追加 `-2`、`-3`。
- 生成的 id 用于 TOC 链接和正文 heading。

inline Markdown 清理规则：

- 去掉 emphasis / strong 标记：`**重点**` → `重点`。
- 链接使用可见文本：`[标题](https://example.com)` → `标题`。
- inline code 使用 code 文本：`` `code` `` → `code`。
- 去掉残留反斜杠转义符。
...
- heading id 按正文出现顺序消费 `toc` 队列，不按标题文本查找，避免重复标题全部匹配到同一个 id。

这样 TOC 提取和 Markdown 渲染共享同一套结果，避免两边 slug 不一致。
所有输出到 HTML 的 TOC 文本、heading id、`href` 都必须经过 HTML escape / attribute escape。
...
本次实现应移除 `renderLibrary(items, selectedCapture)` 以及仅服务该分栏详情模型的 `renderSidebar` / `renderReader` helper，并同步删除对应测试，避免未来维护两套详情模型。
...
- 保留已有视频渲染能力。
- 移除 `renderLibrary` 后，同步删除对应测试，避免死代码测试。
