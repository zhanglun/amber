...
- [ ] `index.ts` 导出 `runDoctor` / `formatDoctorResult`（或 `./doctor` 子路径 export）
- [ ] 全新环境验证 git 依赖安装 + `prepare` 构建成功
- [ ] 打 tag 并推送

**amber 侧：**

- [ ] `dino` 依赖改为 `github:zhanglun/dino#<tag>`，删除 `file:` 路径依赖
- [ ] 新增 `amber doctor` 命令（包 dino `runDoctor`）
- [ ] 需要浏览器的 import 路径：主动检测 Chromium，缺失给"运行 `amber doctor`"的可读引导
- [ ] README / configuration 文档：安装步骤 + 动态站浏览器说明
- [ ] 单测：doctor 命令、浏览器缺失时的错误引导路径
