...
- [ ] **步骤 1：创建 `packages/domain/package.json`**
...
- [ ] **步骤 2：创建 `packages/domain/tsconfig.json`**
...
- [ ] **步骤 3：创建 `packages/domain/src/index.ts`**
...
- [ ] **步骤 3：编写失败的测试 `packages/core/src/asset-key.test.ts`**
...
- [ ] **步骤 5：实现 `packages/core/src/asset-key.ts`**
...
- [ ] **步骤 1：编写失败的测试 `packages/core/src/import-service.test.ts`**
...
- [ ] **步骤 3：实现 `packages/core/src/import-service.ts`**
...
- [ ] **步骤 1：编写失败的测试 `packages/core/src/read-service.test.ts`**
...
- [ ] **步骤 3：实现 `packages/core/src/read-service.ts`**
...
- [ ] **步骤 4：创建 `packages/core/src/index.ts`**
...
- [ ] **步骤 1：创建 `packages/adapters/package.json`**
...
- [ ] **步骤 2：创建 `packages/adapters/tsconfig.json`**
...
- [ ] **步骤 3：创建 `packages/adapters/prisma/schema.prisma`**
...
- [ ] **步骤 1：实现 `packages/adapters/src/r2-blob-store.ts`**
...
- [ ] **步骤 1：编写失败的测试 `packages/adapters/src/dino-source.test.ts`**
...
- [ ] **步骤 3：实现 `packages/adapters/src/dino-source.ts`**
...
- [ ] **步骤 4：创建 `packages/adapters/src/index.ts`**
...
- [ ] **步骤 1：创建 `packages/cli/package.json`**
...
- [ ] **步骤 2：创建 `packages/cli/tsconfig.json`**
...
- [ ] **步骤 3：创建 `packages/cli/src/wiring.ts`**
...
- [ ] **步骤 4：创建 `packages/cli/src/commands/import.ts`**
...
- [ ] **步骤 5：创建 `packages/cli/src/main.ts`**
...
- [ ] **步骤 3：编写失败的测试 `packages/web/src/render.test.ts`**
...
- [ ] **步骤 5：实现 `packages/web/src/render.ts`**
...
- [ ] **步骤 7：实现 `packages/web/src/index.ts`**
...
- [ ] **步骤 0：在 `packages/cli/package.json` 的 dependencies 加入 `@amber/web`**
...
- [ ] **步骤 1：创建 `packages/cli/src/commands/serve.ts`**
...
- [ ] **步骤 2：创建 `packages/cli/src/ui/CaptureList.tsx`**
...
- [ ] **步骤 3：创建 `packages/cli/src/commands/list.ts`**
...
- [ ] **步骤 4：修改 `packages/cli/src/main.ts`，注册全部命令**
...
- [ ] **步骤 6：手动端到端验证（需要真实 .env）**

把 `.env.example` 复制为 `.env`，填入真实的 Supabase + R2 值，然后：

```bash
pnpm --filter @amber/adapters prisma:migrate --name init   # 创建 captures 表
...
```
预期：导入成功；list 显示该条目；浏览器显示列表，点击后打开文章，图片从 R2 加载。
...
- [ ] `amber import <url>`：正文存入 Postgres，图片存入 R2 且链接已改写（任务 4 逻辑 + 任务 11 步骤 6）。
- [ ] `amber serve`：在 `localhost:7788` 显示列表（任务 10 + 任务 11）。
- [ ] 点击列表条目，打开排版干净、带图片的文章页（任务 10）。
- [ ] （可选）另一台机器连同一个 Supabase/R2 能看到相同内容 —— 这是云端存储的天然副产品，非硬性门槛。
```
